...
def latex_to_image(latex_str):
    ...
    if not latex_str or latex_str.strip() == '$':
        logger.error("Invalid or empty LaTeX string for image generation")
        return None
    try:
...
def latex_to_pdf(latex_str):
    ...
    if not latex_str or latex_str.strip() == '$':
        logger.error("Invalid or empty LaTeX string for PDF generation")
        return None
    try:
        char_count = len(latex_str)
        fontsize = min(20, max(12, 20 - char_count // 10))
...
def append_to_formula(symbol):
    ...
    temp_formula = st.session_state.get('pending_formula', st.session_state.formula)
    mode = st.session_state.mode
    ...
    temp_formula += text
    st.session_state.pending_formula = temp_formula
    update_latex()
...
def add_subscript(subscript, selected_param):
    ...
    if not re.match(r'^[\w\d]+$', subscript):
        ...
        return
    formula = st.session_state.get('pending_formula', st.session_state.formula)
    if not formula.strip():
    ...
    if param_positions:
        ...
        st.session_state.pending_formula = new_formula
