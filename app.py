...
st.text_input("Enter formula (e.g., x^2 + sqrt(y) for SymPy, or x^{2} + \\sqrt{y} for LaTeX)", key="formula", on_change=lambda: update_latex(st))
...
if parameters:
    ...
    if st.button("Apply Subscript", key="apply_subscript"):
        add_subscript(st, subscript_input, selected_param)
...
# Symbol definitions moved to symbols.py
...
for idx, tab in enumerate(tabs[:9]):  # Basic to Petroleum
    with tab:
        ...
        num_cols = min(5, len(symbols)) if isinstance(symbols, list) else min(5, len(symbols))
        cols = st.columns(num_cols) if num_cols > 0 else [st]
        if tab_names[idx] == "Calculus":
            for i, (label, texts) in enumerate(symbols.items()):
                col_idx = i % num_cols
                with cols[col_idx]:
                    text = texts[st.session_state.mode]
                    if st.button(label, key=f"btn_{tab_names[idx]}_{i}"):
                        append_to_formula(st, text)
        else:
            for i, (label, text) in enumerate(symbols):
                col_idx = i % num_cols
                with cols[col_idx]:
                    if st.button(label, key=f"btn_{tab_names[idx]}_{i}"):
                        append_to_formula(st, text if not isinstance(text, dict) else text[st.session_state.mode])
...
if st.session_state.latex and not st.session_state.latex.startswith("Invalid") and not st.session_state.latex.startswith("Parse error"):
    ...
    """
    components.html(html_content, height=400)  # Increased height for longer formulas
else:
